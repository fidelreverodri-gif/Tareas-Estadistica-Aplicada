...
author: Reveles Rodríguez Fidel Fernando
format: 
  html:
    grid: 
      body-width: 950px
...
Este conjunto de datos para el estado de Jalisco incluye: - Límites estatales (archivos `32ent.*`) - Límites municipales (archivos `32mun.*`) - Otras divisiones territoriales
...
jalisco_estado <- st_read("data/32ent.shp", options = "ENCODING=LATIN1", quiet = TRUE)
...
```{r municipios}
#| code-fold: true
municipios_jalisco <- st_read("data/32mun.shp", options = "ENCODING=LATIN1", quiet = TRUE)

# Explorar los datos
cat("Número de municipios en Zacatecas:", nrow(municipios_jalisco), "\n")
...
mapa_destacados <- ggplot(municipios_jalisco) +
  ...
  labs(
    title = "Municipios Importantes de Zacatecas",
...
print("Los 10 municipios más grandes de Zacatecas:")
...
mapa_areas <- ggplot(municipios_jalisco) +
  ...
  labs(
    title = "Área de los Municipios de Zacatecas",
