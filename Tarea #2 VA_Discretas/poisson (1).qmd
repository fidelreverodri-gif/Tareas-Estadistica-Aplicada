...
hist(S, breaks=(min(S):(max(S)+1))-0.5, probability=TRUE,
     col="#9ACD32", main="Suma de Poisson X1+X2 vs Poisson(6.3)",
     xlab="X1 + X2")

# Distribución teórica Poisson(6.3)
x_vals <- 0:max(S)
lines(x_vals, dpois(x_vals, lambda=lambda1+lambda2), col="#0000CD", lwd=2)

legend("topright", legend=c("Simulación","Poisson teórica"),
       col=c("#9ACD32","#0000CD"), lwd=c(10,2), bty="n")
...
hist(visitantes, breaks=0:max(visitantes), col="#7FFF00", main="Visitantes por minuto (1 hora)",
     xlab="Número de visitantes")
abline(v=cap1, col="#0000FF", lwd=2, lty=2)
legend("topright", legend=c("Capacidad del servidor"), col="#0000FF", lwd=2, lty=2)
