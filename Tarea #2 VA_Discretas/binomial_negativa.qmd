...
:::
```{r}
neg_binomial_comparacion <- function(r, p, n_sim = 100000) {
  set.seed(456)
  ...
  lines(x_vals, dnbinom(x_vals, size = r, prob = p), col="#66CD00", lwd=2)
  legend("topleft", legend=c("PMF teórica Binomial Negativa"), col="#66CD00", lwd=2)
...
# --- Ejemplo de uso ---
resultado <- neg_binomial_comparacion(r = 10, p = 0.5, n_sim = 10000)
...
hist(fallos, breaks = (min(fallos):(max(fallos)+1))-0.5, probability = TRUE,
     col = "#40E0D0", main = "Tiros fallados antes de 8 éxitos", xlab = "Número de fallos")

x_vals <- 0:max(fallos)
lines(x_vals, dnbinom(x_vals, size=r, prob=p), col="#FF8247", lwd=2)
legend("topright", legend=c("Simulación","Binomial Negativa teórica"),
       col=c("#40E0D0","#FF8247"), lwd=c(10,2))
