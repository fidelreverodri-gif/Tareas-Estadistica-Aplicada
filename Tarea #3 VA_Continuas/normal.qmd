...
dist_altura <- tibble(
  estaturas = seq(mu_altura - 4 * sigma_altura,
                  mu_altura + 4 * sigma_altura,
                  length = 1000)
) |> 
  mutate(
...
ggplot(dist_altura) +
  geom_line(aes(estaturas, pdf_altura), color = "#333AAA", 
            linewidth = 1.3) +
  geom_area(data = dist_altura |> filter(estaturas >= 180),
            aes(estaturas, pdf_altura), fill = "green", alpha = 0.5) +
  geom_vline(xintercept = 180, linetype = "dashed") +
  annotate("text", x = 190, y = 0.01,
           label = paste("1 - F(180)")) +
  theme_bw()
```
```
...
mu <- 350        # media nominal
sigma <- 5       # desviación estándar
n <- 1000        # número de mediciones
lim_inf <- 340   # límite inferior de especificación
lim_sup <- 360   # límite superior de especificación
...
# Guardamos en tibble
datos <- tibble(temperatura = temperaturas)

### 2. ¿Qué porcentaje del tiempo la temperatura está fuera de especificación?
fuera_especificacion <- mean(temperaturas < lim_inf | temperaturas > lim_sup) * 100

### 3. Si el proceso se descentra a mu = 355°C
temperaturas_descentradas <- rnorm(n, mean = 355, sd = sigma)
fuera_especificacion_descentrado <- mean(temperaturas_descentradas < lim_inf | 
                                        temperaturas_descentradas > lim_sup) * 100

### 4. ¿Qué sigma se necesita para tener 99.9% de productos dentro de especificación?
# Queremos: P(340 <= X <= 360) = 0.999 con mu = 350
# Usamos simetría: P(|X - 350| <= 10) = 0.999
# Equivalente a: P(Z <= 10/sigma) - P(Z <= -10/sigma) = 0.999
# Como la normal es simétrica: 2*pnorm(10/sigma) - 1 = 0.999
# Entonces: pnorm(10/sigma) = 0.9995

sigma_requerido <- 10 / qnorm(0.9995)

### Resultados
cat("=== Resultados ===\n")
cat("2. % fuera de especificación (mu = 350, sigma = 5):", round(fuera_especificacion, 2), "%\n")
cat("3. % fuera de especificación (mu = 355, sigma = 5):", round(fuera_especificacion_descentrado, 2), "%\n")
cat("4. Sigma requerido para 99.9% dentro de especificación:", round(sigma_requerido, 2), "\n")
...
ggplot(datos, aes(x = temperatura)) +
  ...
  geom_vline(xintercept = c(lim_inf, lim_sup), linetype = "dashed", color = "blue") +
  annotate("text", x = lim_inf, y = 0.05, label = "Límite Inf", hjust = 1.2, color = "blue") +
  annotate("text", x = lim_sup, y = 0.05, label = "Límite Sup", hjust = -0.2, color = "blue") +
  labs(title = "Distribución de Temperaturas del Proceso",
       subtitle = "Simulación de 1000 mediciones con N(350, 5²)",
...
n <- 24           # número de meses a simular

# 1. Simulación de 24 meses
set.seed(123) # reproducibilidad
ventas <- rnorm(n, mean = mu, sd = sigma)
...
# 2. Probabilidad de tener ventas > 60,000
prob_mayor_60000 <- 1 - pnorm(60000, mean = mu, sd = sigma)

# 3. Meses esperados con ventas < 30,000
prob_menor_30000 <- pnorm(30000, mean = mu, sd = sigma)
meses_esperados_bajos <- prob_menor_30000 * n
...
prob_mayor_45000 <- 1 - pnorm(45000, mean = mu, sd = sigma)

# ============================
# Resultados
# ============================
cat("=== Resultados ===\n")
cat("1. Simulación de ventas (primeros 6 meses):\n")
print(head(datos, 6))
cat("\n2. Probabilidad de ventas > 60,000:", round(prob_mayor_60000*100, 2), "%\n")
cat("3. Meses esperados con ventas < 30,000:", round(meses_esperados_bajos, 2), "meses\n")
cat("4. Rango intercuartílico (IQR):", round(iqr, 2), "\n")
cat("5. % de meses con ventas > 45,000:", round(prob_mayor_45000*100, 2), "%\n")
...
ggplot(datos, aes(x = mes, y = ventas)) +
  ...
  geom_hline(yintercept = 45000, linetype = "dashed", color = "green") +
  annotate("text", x = 2, y = 45500, label = "Meta: 45,000", color = "green", hjust = 0) +
  labs(title = "Simulación de Ventas Mensuales",
       subtitle = "N(50,000 , 12,000²) con 24 meses",
       x = "Mes", y = "Ventas ($)") +
  theme_bw()
...
# Buscamos el valor tal que P(X <= garantia) = 0.95
garantia_95 <- qnorm(0.95, mean = mu, sd = sigma)

### 3. Si se venden 10,000 baterías con garantía de 900 horas
# Probabilidad de que una batería falle antes de 900 horas
prob_falla <- pnorm(900, mean = mu, sd = sigma)
baterias_falla_esperadas <- 10000 * prob_falla

### 4. Comparar media y mediana de la simulación vs valores teóricos
...
# ============================
cat("=== Resultados ===\n")
cat("1. Primeras 6 simulaciones de vida útil:\n")
print(head(datos,6))
cat("\n2. Garantía que cubre 95% de las baterías:", round(garantia_95,2), "horas\n")
cat("3. Baterías que se esperan fallen con garantía de 900 horas:", round(baterias_falla_esperadas,0), "\n")
cat("4. Media simulada:", round(media_sim,2), "vs teórica:", mu, "\n")
cat("   Mediana simulada:", round(mediana_sim,2), "vs teórica:", mu, "\n")
cat("5. Percentil 10 de la distribución:", round(percentil_10,2), "horas\n")
...
ggplot(datos, aes(x = horas)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "skyblue", color = "black") +
  stat_function(fun = dnorm, args = list(mean = mu, sd = sigma), color = "red", linewidth = 1.2) +
  geom_vline(xintercept = garantia_95, linetype = "dashed", color = "blue") +
  annotate("text", x = garantia_95+20, y = 0.0025, label = "95% Garantía", color = "blue", hjust = 0) +
  labs(title = "Simulación de Vida Útil de Baterías",
       subtitle = "N(1200, 150²) con 800 simulaciones",
       x = "Horas de Vida", y = "Densidad") +
  theme_bw()
...
# Parámetros
mu <- 0                # media de los errores
sigma <- 2.5^5          # desviación estándar (según tu nota)
n <- 1000              # número de mediciones
...
### 2. % de errores entre -5 y +5 unidades
pct_entre_5 <- mean(errores >= -5 & errores <= 5) * 100

### 3. Probabilidad de error > 6 unidades en valor absoluto
prob_mayor_6 <- mean(abs(errores) > 6) * 100

### 4. % de mediciones consideradas precisas (error ≤ 3)
pct_preciso <- mean(abs(errores) <= 3) * 100

### 5. Verificar simetría alrededor de 0
# Comparamos la media y mediana de la simulación
media_sim <- mean(errores)
mediana_sim <- median(errores)

# ============================
# Resultados
# ============================
cat("=== Resultados ===\n")
cat("1. Primeras 6 mediciones simuladas:\n")
print(head(datos,6))
cat("\n2. % de errores entre -5 y +5:", round(pct_entre_5, 2), "%\n")
cat("3. % de errores > 6 en valor absoluto:", round(prob_mayor_6, 2), "%\n")
cat("4. % de mediciones precisas (≤3 unidades):", round(pct_preciso, 2), "%\n")
cat("5. Simetría: media =", round(media_sim,4), ", mediana =", round(mediana_sim,4), "\n")

# ============================
# Gráfico de distribución
# ============================
ggplot(datos, aes(x = error)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "green", color = "black") +
  stat_function(fun = dnorm, args = list(mean = mu, sd = sigma), color = "yellow", linewidth = 1.2) +
  geom_vline(xintercept = c(-5,5), linetype = "dashed", color = "blue") +
  geom_vline(xintercept = c(-3,3), linetype = "dashed", color = "green") +
  annotate("text", x = -5, y = 0.00001, label = "-5/+5", color = "blue", hjust = 1.2) +
  annotate("text", x = -3, y = 0.00001, label = "-3/+3", color = "green", hjust = 1.2) +
  labs(title = "Simulación de Errores de Medición",
       subtitle = "N(0, 2.5^5) con 1000 mediciones",
...
mu_B <- 82
sigma_B <- 8
n <- 200  # estudiantes por grupo

### 1. Simulación de 200 estudiantes por grupo
set.seed(123)
puntajes_A <- rnorm(n, mean = mu_A, sd = sigma_A)
puntajes_B <- rnorm(n, mean = mu_B, sd = sigma_B)
...
### 2. % de estudiantes del Grupo A que supera la media del Grupo B
pct_A_supera_B <- mean(puntajes_A > mu_B) * 100

### 3. Probabilidad de que un estudiante del Grupo B tenga puntuación < 70
prob_B_menor_70 <- pnorm(70, mean = mu_B, sd = sigma_B) * 100

### 4. Comparar distribuciones visualmente
datos_long <- datos %>% pivot_longer(cols = c(GrupoA, GrupoB),
                                     names_to = "Grupo", values_to = "Puntaje")

### 5. Puntuación que define el 10% superior de cada grupo
percentil_90_A <- qnorm(0.90, mean = mu_A, sd = sigma_A)
percentil_90_B <- qnorm(0.90, mean = mu_B, sd = sigma_B)

# ============================
# Resultados
# ============================
cat("=== Resultados ===\n")
cat("1. Primeros 6 estudiantes simulados:\n")
print(head(datos))
cat("\n2. % de estudiantes del Grupo A que supera la media del Grupo B:", round(pct_A_supera_B,2), "%\n")
cat("3. Probabilidad de estudiante del Grupo B < 70:", round(prob_B_menor_70,2), "%\n")
cat("5. Puntuación 10% superior Grupo A:", round(percentil_90_A,2), "\n")
cat("   Puntuación 10% superior Grupo B:", round(percentil_90_B,2), "\n")

# ============================
# Gráfico comparativo
# ============================
ggplot(datos_long, aes(x = Puntaje, fill = Grupo)) +
  geom_histogram(aes(y = ..density..), bins = 20, alpha = 0.5, position = "identity", color = "red") +
  stat_function(fun = dnorm, args = list(mean = mu_A, sd = sigma_A), color = "blue", linewidth = 1.2) +
  stat_function(fun = dnorm, args = list(mean = mu_B, sd = sigma_B), color = "green", linewidth = 1.2) +
  geom_vline(xintercept = c(percentil_90_A, percentil_90_B), linetype = "dashed", color = c("blue","green")) +
  annotate("text", x = percentil_90_A+1, y = 0.015, label = "10% Sup A", color = "blue") +
  annotate("text", x = percentil_90_B+1, y = 0.015, label = "10% Sup B", color = "green") +
  labs(title = "Distribución de Puntuaciones: Grupo A vs Grupo B",
